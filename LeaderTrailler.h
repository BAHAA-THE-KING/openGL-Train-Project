...
private:
	...
	void DrawHolocaust(bool *keys, Point pos)
	{
		...
		glPopMatrix();
		glTranslated(0,Holocaust_hight_cylinder+1,-Holocaust_rad_cylinder);
		glBindTexture(GL_TEXTURE_2D,texture_smoke);
		glBegin(GL_POLYGON);
		for(float angle=0; angle<2*3.14; angle+=2*3.14/20.0)
		{
			glTexCoord2f(0.5+cos(angle)/2.0,0.5+sin(angle)/2.0);
			glVertex3f(cos(angle),sin(angle),0);
		}
		glEnd();
		float T0=depth_cube/2, step =T0/20.0;
		glEnable(GL_BLEND);
		glColor4f(1,1,1,1);
		glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
		...
		for (int i=1; i<=3; i++)
		{
			for (float t=0; t<T0*0.6; t+=step)
			{
				float z=t+(i-1)*T0*0.6, rad=i+abs(sin(3.14*t/T0)), next_rad=i+abs(sin(3.14*(t+step)/T0));
				premitives::DrawCylinderBody(Point(0,0,z),rad, next_rad,step,texture_smoke);
			}
		}
		glDisable(GL_BLEND);
		glTranslated(0,0,3*T0*0.6);
		float rad=3+abs(sin(3.14*0.6));
		glBindTexture(GL_TEXTURE_2D,texture_smoke);
		glBegin(GL_POLYGON);
		for(float angle=0; angle<2*3.14; angle+=2*3.14/20.0)
		{
